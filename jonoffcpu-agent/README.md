...
flavour. `-Dio.github.jonoffcpu.jonoffcpu.nativeLibc=glibc` or `=musl` overrides
the detection.
...
`io.github.jonoffcpu.jonoffcpu.agent.v1.Manifest` message in
...
`io.github.jonoffcpu.jonoffcpu.agent.SignalCaptureAgent.start(yamlOrConfigPath)` and later
...
Applications may call `io.github.jonoffcpu.jonoffcpu.agent.SignalCaptureAgent.stop()` for reliable
