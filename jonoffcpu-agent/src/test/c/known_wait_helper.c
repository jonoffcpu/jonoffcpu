...
Java_io_github_lhotari_jonoffcpu_agent_KnownWaitHelper_currentTid(JNIEnv *env, jclass type) {
    (void)env;
    (void)type;
...
JNIEXPORT jlong JNICALL
Java_io_github_lhotari_jonoffcpu_agent_KnownWaitHelper_monotonicNanos(JNIEnv *env, jclass type) {
    (void)type;
    struct timespec now;
...
Java_io_github_lhotari_jonoffcpu_agent_KnownWaitHelper_nativeWaitNanos(JNIEnv *env, jclass type, jlong duration_nanos) {
    (void)env;
    (void)type;
    if (duration_nanos <= 0) {
        return EINVAL;
