...
4. A [native collector](jonoffcpu-native/src/collector.rs) in the JVM process
   ...
   and SHA-256.
5. [`OffCpuCorrelator`](jonoffcpu-correlator/src/main/java/io/github/jonoffcpu/jonoffcpu/offline/OffCpuCorrelator.java)
...
`-Dio.github.jonoffcpu.jonoffcpu.nativeLibc=glibc` or `=musl` selects it
explicitly. If the temporary directory is mounted `noexec`, point
`-Dio.github.jonoffcpu.jonoffcpu.nativeWorkDir` at an executable location.
...
| An executable temporary directory | `-Dio.github.jonoffcpu.jonoffcpu.nativeWorkDir=…` if `/tmp` is `noexec` | The agent extracts the native bundle and executes it. |
...
calls `io.github.jonoffcpu.jonoffcpu.agent.SignalCaptureAgent.stop()`. Abrupt
