 ...
 *
 * <p>{@code osThreadId} is duplicated out of the thread dictionary because the join's thread-identity
...
final class SampleColumns {
    ...
    void add(long cookieBits, long monotonic, int tid, int stack, int thread) {
    ...
    long retainedBytes() {
        return (long) cookie.length * (Long.BYTES * 2 + Integer.BYTES * 3 + 2);
    }
