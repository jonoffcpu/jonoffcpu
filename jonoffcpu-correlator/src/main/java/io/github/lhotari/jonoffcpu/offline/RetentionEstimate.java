 ...
 * 1,121,418 {@code profiler.SignalSample} events (35 bytes each). A source column slot is 51 bytes
 * (38 before the stack profile kept each row's reason, task state and native stack ids), a sample slot 38, and a cookie index entry 24 at the index's half load factor. Interned stacks
...
record RetentionEstimate(long sourceBytes, long jfrBytes, long observations, long samples, long retainedBytes) {
    ...
    private static final long SOURCE_SLOT_BYTES = 51;
