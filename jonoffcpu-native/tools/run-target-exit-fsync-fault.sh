...
source_output=${JONOFFCPU_TARGET_EXIT_OUTPUT:-/evidence/jonoffcpu-target-exit-fsync-fault-source.ndjson}
...
docker run --rm --privileged \
  ...
  -e JONOFFCPU_EXPECT_TERMINAL_SYNC_RETRY=1 \
  -w /work/jonoffcpu-native \
  "$image" \
  sh -eu -c '
    cargo build --locked --bin jonoffcpu-target-exit-proof
    cc -shared -fPIC -O2 -Wall -Wextra -Werror -o /tmp/fsync-eio-once.so tools/fsync-eio-once.c -ldl
    LD_PRELOAD=/tmp/fsync-eio-once.so target/debug/jonoffcpu-target-exit-proof
  '
